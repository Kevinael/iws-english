...
import numpy as np
import pytest
from core.sim_diagnostics import (
    Insight,
    generate_insights,
...
def test_balanco_torque_presente(mp):
    ...
    assert any("Balanço" in t for t in titulos)


def test_tipo_info_balanco(mp):
    res = _make_res(s_ss=0.03, Te_ss=12.0)
    ins = generate_insights(res, mp, load_torque=10.0, tmax=3.0, exp_type="dol")
    info = [i for i in ins if "Balanço" in i.title]
...
def test_escorregamento_critico_gera_error(mp):
    ...
    assert any(i.level == "error" and "Sobrecarga" in i.title.upper() or "SOBRECARGA" in i.title for i in ins)
...
def test_escorregamento_alerta_gera_warning(mp):
    ...
    assert any(i.level == "warning" and "Escorregamento" in i.title for i in ins)


def test_escorregamento_normal_sem_sobrecarga(mp):
    res = _make_res(s_ss=0.03, Te_ss=12.0)
    ins = generate_insights(res, mp, load_torque=10.0, tmax=3.0, exp_type="dol")
    assert not any("Sobrecarga" in i.title or "SOBRECARGA" in i.title for i in ins)
...
def test_subcarga_gera_warning(mp):
    ...
    assert any("Subcarga" in i.title for i in ins)


def test_sem_subcarga_com_s_normal(mp):
    res = _make_res(s_ss=0.03, Te_ss=12.0)
    ins = generate_insights(res, mp, load_torque=10.0, tmax=3.0, exp_type="dol")
    assert not any("Subcarga" in i.title for i in ins)
...
def test_ampla_reserva_gera_info(mp):
    res = _make_res(s_ss=0.03, Te_ss=12.0)
    # Te_max no array é 24 N·m (dobro de Te_ss), carga = 10 → ratio = 2.4
    ins = generate_insights(res, mp, load_torque=10.0, tmax=3.0, exp_type="dol")
    assert any("Ampla Reserva" in i.title for i in ins)


def test_partida_pesada_gera_error(mp):
    # Te_max ≈ 2×Te_ss = 12, carga = 11 → ratio ≈ 1.09 < 1.2
    res = _make_res(s_ss=0.03, Te_ss=6.0)
    ins = generate_insights(res, mp, load_torque=11.0, tmax=3.0, exp_type="dol")
    assert any("TRAVAMENTO" in i.title or "Pesada" in i.title for i in ins)
...
def test_shutdown_sem_balanco(mp):
    ...
    assert not any("Balanço" in i.title for i in ins)
...
def test_gerador_sem_sobrecarga(mp):
    ...
    assert not any("Sobrecarga" in i.title for i in ins)
