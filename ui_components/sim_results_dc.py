...
# ─────────────────────────────────────────────────────────────────────────────
# SUB-RENDERERS (one per tab + PDF export)
# ─────────────────────────────────────────────────────────────────────────────

def _render_dc_tab_overview(
    res: dict,
    mp: DCMachineParams,
    exc: str,
    is_gen: bool,
...
def _render_dc_tab_diagnostics(
    res: dict,
    mp: DCMachineParams,
    exc: str,
    d: int,
...
def render_results_dc(
    ...
    **kwargs,
    ...
    with tab_visao:
        _render_dc_tab_overview(res, mp, exc, is_gen, d, energy_tariff)

    with tab_dinamica:
        _render_dc_tab_dynamic(res, var_keys, var_labels, dark, t_events, d, exp_type, mp, ref_list, exc, _cache_key)

    with tab_diag:
        _render_dc_tab_diagnostics(res, mp, exc, d)

    with tab_ativos:
        _render_dc_tab_assets(res, mp, exc, is_gen, d, h, energy_tariff)
